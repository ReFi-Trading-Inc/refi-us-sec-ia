...
| Which service receives customer onboarding data?              | **Not defined in alpha.3.** No BFF or backend operation receives applicant PII; the package forbids sending raw answers/documents to the trading backend. Where applicant data goes depends on the boundary Daniel binds (question 1).                                                                                                    |
...
derived from a backend binding is **his call** (packet question 4, "K1"). Until answered the attestation chain keeps stopping at
