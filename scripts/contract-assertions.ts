...
// ─── Brokerage onboarding projection (Alpaca-owned KYC/CIP, decision 2026-09-10) ──

const {
  BROKERAGE_ONBOARDING_STATES,
  CONTRACT_BACKED_BROKERAGE_ONBOARDING_STATES,
  UNBOUND_BROKERAGE_ONBOARDING_STATES,
  BROKERAGE_ONBOARDING_LABELS,
  projectBrokerageOnboarding,
} = await import("../apps/web/src/lib/brokerage/onboarding-projection.ts");

await section(
  "brokerage onboarding projection: vocabulary is the union of backed and unbound states, disjoint",
  async () => {
    const backed =
      CONTRACT_BACKED_BROKERAGE_ONBOARDING_STATES as readonly string[];
    const unbound = UNBOUND_BROKERAGE_ONBOARDING_STATES as readonly string[];
    assert.deepEqual([...backed, ...unbound], [...BROKERAGE_ONBOARDING_STATES]);
    assert.deepEqual(
      backed.filter((s) => unbound.includes(s)),
      [],
    );
    for (const s of [
      "not_started",
      ...
      assert.ok(
        (BROKERAGE_ONBOARDING_STATES as readonly string[]).includes(s),
        `mandate §8 state missing: ${s}`,
      );
    }
    assert.deepEqual(
      Object.keys(BROKERAGE_ONBOARDING_LABELS).sort(),
      [...BROKERAGE_ONBOARDING_STATES].sort(),
    );
  },
);

await section(
  "brokerage onboarding projection: labels describe the brokerage state, never an identity/KYC claim",
  async () => {
    const forbidden =
      /sec verified|kyc certified|kyc verified|fully verified|refi approved your identity|identity verified/i;
    for (const [state, label] of Object.entries(BROKERAGE_ONBOARDING_LABELS)) {
      assert.ok(
        !forbidden.test(label),
        `forbidden wording in label for ${state}: ${label}`,
      );
      assert.ok(
        /brokerage|information required/i.test(label),
        `label must name the brokerage state (${state}): ${label}`,
      );
    }
  },
);
...
await section(
  "brokerage onboarding projection: every alpha.3 status pair maps to a contract-backed state; unbound states are never emitted; unknown status fails closed",
  async () => {
    ...
    assert.deepEqual(
      schemaDefs.BrokerageConnection.properties.connection_status.enum,
      [...connectionStatuses],
    );
    assert.deepEqual(
      schemaDefs.BrokerageConnection.properties.credential_status.enum,
      [...credentialStatuses],
    );
    const backed =
      CONTRACT_BACKED_BROKERAGE_ONBOARDING_STATES as readonly string[];
    const seen = new Set<string>();
    const none = projectBrokerageOnboarding(null);
    assert.equal(none.state, "not_started");
    assert.equal(none.source, "none");
    assert.equal(none.connectionUsable, false);
    for (const connection_status of connectionStatuses)
      for (const credential_status of credentialStatuses) {
        const p = projectBrokerageOnboarding({
          connection_status,
          credential_status,
        });
        assert.ok(
          backed.includes(p.state),
          `${connection_status}/${credential_status} → ${p.state} is not contract-backed`,
        );
        assert.ok(
          !(UNBOUND_BROKERAGE_ONBOARDING_STATES as readonly string[]).includes(
            p.state,
          ),
        );
        assert.equal(p.source, "brokerage_connection");
        assert.equal(p.connectionUsable, p.state === "account_active");
        assert.equal(p.label, BROKERAGE_ONBOARDING_LABELS[p.state]);
        seen.add(p.state);
      }
    assert.equal(
      projectBrokerageOnboarding({
    ...
    for (const s of [
      ...
      assert.ok(seen.has(s), `state never produced: ${s}`);
    assert.throws(
      () =>
        projectBrokerageOnboarding({
          connection_status: "APPROVED" as never,
          credential_status: "VALID",
        }),
      /contract drift/,
    );
