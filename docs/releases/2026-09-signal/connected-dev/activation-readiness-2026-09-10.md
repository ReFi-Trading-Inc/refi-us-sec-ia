...
| KYC evidence → compliance attestation                | CODE COMPLETE                     | PR E: trusted evidence only from a final provider decision; alpha.3 `kyc` block carries passed/failed + adapter label + workflow level + opaque ref — the attestation transmits evidence; whether Daniel requires a separate canonical admission state remains a contract dependency (packet 8/9a) |
