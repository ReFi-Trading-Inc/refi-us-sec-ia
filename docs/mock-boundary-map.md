...
| Broker connect + account/positions      | **BFF → contract** | `POST/GET /api/v1/investor/broker/connection` → `createBrokerageConnection` / `listBrokerageConnections`; positions via `/api/v1/investor/portfolio`. The `/v1/brokers/*` MSW fixtures were deleted 2026-09-05. |
