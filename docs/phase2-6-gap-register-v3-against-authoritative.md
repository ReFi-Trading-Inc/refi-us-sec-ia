...
| GAP-MODE-004        | ExecutionPolicy ownership unresolved          | Closed — does not exist; AccountPrefs History gap opens instead                              |
| GAP-EX-002          | ExecutionPolicy is frontend-side only         | Closed — confirmed no backend equivalent; Surface 4 reframed                                 |
...
| **GAP-TRIPWIRE-016** | Tripwire forbidden-term list update: drop `policy_id` / `policy_version` (they don't exist anywhere); admin-shape items remain forbidden.                                           | Low      | Frontend | No                  | No                   | PR-C               |
| **GAP-OPENAPI-017**  | Generated OpenAPI client (`packages/api-clients/src/generated/api.ts` + `api.gen.ts`) still carries `execution_policy_id` / `execution_policy_version` / `strategy_id`. Regenerate. | Low      | Frontend | No                  | No                   | PR-C               |
