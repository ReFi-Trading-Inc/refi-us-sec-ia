...
def _default_root() -> Path:
    ...
    if (candidate / "bundle.json").is_file():
        return candidate
    return Path(__file__).resolve().parents[2] / "contracts/frontend/v1.1.0-alpha.3"
...
def _validate_bundle_manifest(bundle: dict[str, Any]) -> list[str]:
    ...
    if bundle.get("generator") != {
        "path": "scripts/contracts/build_investor_alpha3_handoff.py",
        "version": "2",
    }:
    ...
    if not isinstance(source, dict) or set(source) != {
        ...
        if source.get("path") != (
            "contracts/frontend/investor-api-v1.1.0-alpha.3.json"
        ):
...
def _validate_capability_register(
    ...
    if connection.get("logical_values") != {'project_id': 'refinity-dev', 'project_number': '182665799543', 'region': 'us-west1', 'identity_result_issuer': 'urn:refinity:identity-ccid:dev', 'identity_result_audience': 'urn:refinity:frontend-bff:dev', 'bff_assertion_issuer': 'urn:refinity:bff:dev', 'bff_assertion_audience': 'urn:refinity:investor-api:dev', 'identity_ccid_google_oidc_audience': 'https://identity-ccid.dev.refi.internal', 'investor_api_google_oidc_audience': 'https://investor-api.dev.refi.internal', 'frontend_bff_base_url': 'https://bff-dev.refi.trading', 'backend_identity_jwks_path': '/.well-known/jwks.json', 'runtime_auth_mode': 'native_cloud_run'}:
        errors.append("connection template logical values differ")
    if connection.get("issuance_observations") != {'observed_at': '2026-09-09', 'frontend_bff_base_http_status': 200, 'frontend_bff_jwks_http_status': 503, 'wif_pool_state': 'ACTIVE', 'wif_provider_count': 0, 'identity_ccid_cloud_run_ready': True, 'identity_ccid_feature_state': 'disabled', 'identity_ccid_public_jwks_http_status': 200, 'investor_api_cloud_run_ready': True, 'investor_api_feature_state': 'disabled'}:
...
def validate_bundle(root: Path | None = None) -> list[str]:
    ...
    for token in (
        ...
        "frontend_native_binding",
        "frontend_runtime_service_account_email",
        "frontend_runtime_service_account_unique_id",
