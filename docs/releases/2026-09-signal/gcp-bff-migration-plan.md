...
| Outbound to identity-ccid / Investor API | ID token from the metadata server (`format=full`), audience = the target's configured **Cloud Run custom audience** (`https://identity-ccid.dev.refi.internal`, `https://investor-api.dev.refi.internal`) — never the service URL (corrected 2026-09-09 per Daniel's integration list; see §5), signed by the **BFF runtime service account**                                                         |
