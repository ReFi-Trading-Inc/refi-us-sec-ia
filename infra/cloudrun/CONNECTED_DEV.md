...
- Stytch/Socure are `unconfigured`, not mock. Frontend-owned provider credentials
  ...
  A development-only KYC-pass source exists on `integration/refinity-dev`
  (`apps/web/src/lib/integration-dev/kyc-pass.ts`). It is **not adopted on the
  handoff line** — see
  [`alpha4-reconciliation.md` §6.2](../../docs/daniel-integration-handoff/alpha4-reconciliation.md)
  — and this deployment configuration neither enables nor requires it. Nothing
  here simulates Stytch login or certifies real KYC.
