...
/**
 * Test-only: drop the resolved cache so the next read re-reads process.env.
 * Mirrors rateLimit.__resetForTests(); never called from app code — flags
 * are boot-resolved on purpose in every real environment.
 */
