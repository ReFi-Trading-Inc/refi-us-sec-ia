...
This is the ordered integration work list accompanying **v1.1.0-alpha.4**.
...
current integration target. Alpha.4 retains the corrected error profiles and the
native-Cloud-Run topology; methods, paths, request fields, JWT claims and event shapes are unchanged. Funding responses and recommendation read schemas change as specified in MIGRATION.md.

**Handoff instructions:** deliver this Markdown file together with the complete
`v1.1.0-alpha.4/` directory, including `tools/`. The team currently has alpha.2
at `packages/api-clients/contracts/investor-api/v1.1.0-alpha.2`; vendor alpha.4
alongside it and update package.ts, validation.ts, generation commands and
version/hash assertions to select alpha.4. References to
...
package byte for byte. It must now upgrade to alpha.4. No frontend files were
...
Historical September 9 alpha.3 repairs deployed as the following revisions (not alpha.4 rollout evidence):
...
- [ ] **Adopt the backend's alpha.4 correction.** Alpha.2 already declares
...
can run, alpha.4 is vendored and the acknowledgment adapter/confirmation flow
...
```json
{
  "contract_version": "v1.1.0-alpha.4",
  "package_content_sha256": "COPY_FROM_THIS_PACKAGE_BUNDLE_JSON",
...
| B2 | Supplied by alpha.4: corrected error profiles, runtime status/error mapping, exact bundle digest, MIGRATION.md and local conformance tooling. Eleven real-domain HTTP response cases pass the strict TypeScript client; continuation-preserving adapter change is separately proved in a temporary copy. Frontend vendors/regenerates alpha.4 and applies the adapter change in step 4. No new backend decision or confirmation fields are awaited. | Expanding/mixed preference branch and full contract integration acceptance. |
