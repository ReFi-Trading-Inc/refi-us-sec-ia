...
| **AccountAuthorization**                | **Contract shape already exists** — `PENDING / AUTHORIZED / DENIED / SUSPENDED` with reason codes, policy version and state version (verified in alpha.3; byte-identical in alpha.4 — `lane-f-alpha4-audit.md` F1/F4). Not a shape to invent.                                                                                                                                                                                | Connected policy/acceptance and successor-contract verification                                                                |
...
On `main`, `contracts/investor-api/CURRENT.json` pins **`v1.1.0-alpha.3`** with
`package_content_sha256` provenance and, notably,
**`"connected_alpha_verified": false`**. On the handoff line
(`daniel-handoff/integration`, since #155 / `019a6bf` on 2026-09-13) it pins
**`v1.1.0-alpha.4`** (`a6db935b…`), which is the contract authority for
certification; `main` stays on alpha.3 during the release freeze by design.
