...
| **GAP-RISK-BINARY-006**       | ✅ **RESOLVED 2026-07-30.** REVIEW removed from the OpenAPI `OrderPreviewResult.status` enum, `CompliancePreview.tsx`, and `compliancePreviewCopy`; the manual-review escalation CTA is gone. A contract assertion now pins the wire enum to `[ALLOW, DENY]`.                                              | High     | Frontend                               | Surface 9                       | No (cleanup)        | No                                                                | PR-C                              | PR-C realignment                                                                                                   |
